...
contract('TaskBook', function(accounts) {
  ...
  it("should assert true", function() {
    ...
    TaskBook.deployed().then(function(_task_book) {
    	...
    	return task_book.submitDeposit({from: accounts[1], value: 10000});
    }).then(function(tx) {
    	//TaskGiver
    	assert.equal(web3.utils.soliditySha3(accounts[1]), tx.receipt.logs[0].data);
    	task_giver = accounts[1];
    	return task_book.submitDeposit({from: accounts[2], value: 10000});
    }).then(function(tx) {
    	//Solver
    	assert.equal(web3.utils.soliditySha3(accounts[2]), tx.receipt.logs[0].data);
    	solver = accounts[2];
    	return task_book.submitDeposit({from: accounts[3], value: 10000});
    }).then(function(tx) {
    	//Verifier
